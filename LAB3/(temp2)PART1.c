...
void GPIOinit()
{
    GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P2, GPIO_PIN7, GPIO_PRIMARY_MODULE_FUNCTION);  // Left Motor PWM
    GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P2, GPIO_PIN6, GPIO_PRIMARY_MODULE_FUNCTION);  // Right Motor PWM
...
void timerInit()
{
    ...
    // Right Wheel PWM
    compare_R.compareRegister = TIMER_A_CAPTURECOMPARE_REGISTER_0;
...
void updatePWM()
{
    ...
    Timer_A_setCompareValue(TIMER_A0_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_1, roundf(period * dutycycle_L));
    Timer_A_setCompareValue(TIMER_A0_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_0, roundf(period * dutycycle_R));
}
...
void encoderISR()
{
    if (Timer_A_getCaptureCompareEnabledInterruptStatus(TIMER_A3_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_1) & TIMER_A_CAPTURECOMPARE_INTERRUPT_FLAG)
    {
        ...
        if (count_samples_L == 6)
        {
            ...
            count_samples_L = 0;
            sum_counts_L = 0;
            Timer_A_setCompareValue(TIMER_A3_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_1, roundf(period * dutycycle_L));
        }
    }
    ...
    if (Timer_A_getCaptureCompareEnabledInterruptStatus(TIMER_A3_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_0) & TIMER_A_CAPTURECOMPARE_INTERRUPT_FLAG)
    {
        ...
        if (count_samples_R == 6)
        {
            ...
            count_samples_R = 0;
            sum_counts_R = 0;
            Timer_A_setCompareValue(TIMER_A3_BASE, TIMER_A_CAPTURECOMPARE_REGISTER_0, roundf(period * dutycycle_R));
        }
    }
