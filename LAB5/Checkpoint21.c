...
float kp_angle = 0.002;
float kd_angle = 0.001;
...
int segment_enc_L, segment_enc_R = 0;
...
int main() {    /** Main Function ****/
    ...
    while(1)
    {
        ...
        if(runControl){    // If 100 ms has passed
            ...
            if (SS1 == 1)
            {
                diffSpeed = -roll/256.0;
                printf("Part A\r\n");
            }
            else
            {
                float heading_rm_deg10 = heading_rm * (1800.0 / 3.1415);

                int heading_measured = (arry[0] << 8) | arry[1];

                int heading_error = heading_measured - heading_rm_deg10;
                ...
                int derivative = heading_error - prev_heading_error;
                diffSpeed = kp_angle * heading_error + kd_angle * derivative;
                ...
                else if (diffSpeed < -0.2)
                {
                    diffSpeed = -0.2;
                }

                printf("Part B Angle-based | Err: %d | Δ: %.3f\r\n", heading_error, diffSpeed);
            }
            ...
            prev_enc_L = enc_total_L;
            prev_enc_R = enc_total_R;

            segment_enc_L = 0;
            segment_enc_R = 0;

            printf("deltaTheta: %.4f rad, Heading: %.4f rad\n", deltaTheta, heading_rm);
