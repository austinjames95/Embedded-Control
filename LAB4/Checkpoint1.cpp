...
int main() {    /** Main Function ****/
    ...
    while (1) {
        if (runControl) {
            ...
            potSpeed = ADC14_getResult(ADC_MEM12);   // Speed potentiometer


            float speedNorm = (potSpeed - 8192.0)/8192.0;           // [-1, 1]
            //printf("%f\n", speedNorm);

            float desiredSpeed = speedNorm * vdMax;
            if (fabs(desiredSpeed) < vdMin)
            {
                desiredSpeed = 0;
            }
            ...
            if (TachR_avg > 0)
            {
                speedR_meas = 1500000.0 / TachR_avg;
            }
            else
            {
                speedR_meas = 0.0;
            ...
            // Convert encoder counts to duty cycle unit
            if (speedR_meas > 100.0)
            {
                speedR_meas = 100.0;
            }

            // PI Control
            float errorR = fabs(desiredSpeed) - speedR_meas;
            errorSumR += errorR;

            float controlR = kP * errorR + kI * errorSumR;
            ...
            //printf("PWM=%d Nadc=%d\n", cmpR, desiredSpeed);
            //printf("R: Vd=%.2f Vm=%.2f Err=%.2f Int=%.2f Ctrl=%.2f PWM=%d\n",
                   //desiredR, speedR_meas, errorR, errorSumR, controlR, cmpR);
        }
    }
}    /** End Main Function ****/   
...
void ADCInit(){
    ...
    //ADC14_configureMultiSequenceMode(ADC_MEM12, ADC_MEM9, false);
